# BA / Backend / Frontend Developer integration audit
...
- BA: extracted `role1-sprint1-v2.1/` package found; no matching BA ZIP
...
| Frontend Developer UI | React + TypeScript + Vite; AppShell, routes, plan form, processing, result, review, audit, policy and Verify pages |
| UI service seam | ServicesProvider and interfaces; App.tsx currently instantiates createMockServices unconditionally |
| Verify/seeds | src/verify/harness.py contains five TEST_ONLY scenarios and demo configuration; BA has separate 15 GT and five Verify datasets with fixtures |
...
uploads. BA documents a separate 100m demo limit. These configurations must not
...
3. Frontend Developer exposes REQUEST_CHANGES and Stop/retry processing actions. Existing
...
5. BA fact_verification is explicitly a fixture extension proposal. Its
...
6. Existing TEST_ONLY Verify cases are not the BA dataset. Their successful
   tests must not be reported as BA engine acceptance.
...
2. Add a BA dataset adapter and actual regression tests without changing
...
4. Implement Frontend Developer API services through existing interfaces; connect forms,
...
Expected change areas: new HTTP adapter and tests, BA integration adapter and
...
- Limitations: no API/frontend integration delivered; no BA engine regression
