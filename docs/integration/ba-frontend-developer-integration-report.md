# BA – Backend – Frontend Developer integration report
...
The existing extracted `role1-sprint1-v2.1/` was used without overwriting it. All numbered business documents, validation-report and package-manifest are under `docs/role1/v2.1/`. Fixture JSON and image bytes are under `tests/fixtures/ba/v2.1/`.
...
Detailed [mapping matrix](ba-contract-mapping.md) and [conflict report](ba-conflict-report.md) describe each translation. VND uses exact integer minor units; attachment identity/hash comes from backend-uploaded bytes; agent evidence retains source and issue provenance. Weighted strategy criteria use Decimal. Proposed fact_verification maps into existing missing_facts/evidence_conflicts/evidence, with strict unknown/missing field rejection. No frozen contract or outcome catalog was extended.
...
| python tools/validate_package.py (original BA directory) | 192/192 artifact checks |
...
[Durable results](ba-regression-results.json) include execution times and original actual-report hashes. Full persisted observations are in runtime/role1-ground-truth-actual.json and runtime/role1-verify-actual.json.
...
Production identity provider, real company limits/policy, actual local VLM deployment, a VLM-only confidence gate, kill-switch semantics and other BA proposed policy questions remain owner decisions. They do not block the explicitly authorized synthetic demo. Completion of the GT-014/015 category oracle remains with BA; no category was invented on its behalf.
...
New Verify: src/verify/ba_adapter.py, runner.py, observation.py. Existing src/verify modules preserved.
...
Tests added: tests/integration/test_http_api.py, test_demo_seed.py, test_ba_regression.py; tests/unit/verify/test_ba_mapping.py; tests/fixtures/ba/v2.1/*.

Documentation: docs/integration/*; docs/role1/v2.1/*; docs/role1/archive/v2.1/*; docs/role-integration-audit-2026-09-21.md; tasks/plan.md; tasks/todo.md. BA imports are individually enumerated in the manifest. Existing app.py, src/frontend and their tests were preserved.
