...
export function ReviewQueuePage() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    setLoadError(null)
    try {
      const items = await services.review.listPendingReviews()
      if (requestId === loadRequest.current) setQueue(items)
    } catch {
      if (requestId === loadRequest.current) setLoadError('Không thể tải danh sách hồ sơ cần human review.')
    } finally {
  ...
  return (
    <section className="review-page">
      <PageHeader
        eyebrow="Thẩm định thủ công"
        title="Hồ sơ chờ thẩm định"
        description="Các kế hoạch được backend chuyển tới Checker hiện tại. Chọn một hồ sơ để xem evidence, rule áp dụng và ghi nhận quyết định."
        actions={<button type="button" className="button button-secondary" onClick={() => void loadQueue()} disabled={isLoading}><RefreshCw aria-hidden="true" /> Tải lại</button>}
      />
      ...
      <div className="review-layout">
        <section className="review-queue-panel" aria-label="Danh sách hồ sơ">
          <div className="section-toolbar"><div><p className="page-eyebrow">Hàng chờ của Checker</p><h3>Hồ sơ cần xử lý</h3></div>{!isLoading && !loadError && <span className="readonly-label"><ClipboardCheck aria-hidden="true" /> {queue.length} hồ sơ</span>}</div>
          {isLoading && <StatePanel kind="loading" title="Đang tải hàng chờ" description="Đang lấy các hồ sơ được backend chuyển tới actor hiện tại." />}
          {!isLoading && !loadError && queue.length === 0 && <StatePanel kind="empty" title="Không có hồ sơ chờ thẩm định" description="Backend hiện chưa trả về hồ sơ human review nào cho Checker này." />}
          {!isLoading && queue.length > 0 && <div className="review-table-wrap">
            <table className="review-table">
              <colgroup>
                <col className="review-col-campaign" />
                <col className="review-col-escalation" />
                <col className="review-col-reason" />
                <col className="review-col-priority" />
                <col className="review-col-authority" />
                <col className="review-col-time" />
                <col className="review-col-status" />
              </colgroup>
              <thead><tr><th>Chiến dịch</th><th>Nhóm chuyển tiếp</th><th>Lý do</th><th>Ưu tiên</th><th>Người duyệt</th><th>Thời gian</th><th>Trạng thái</th></tr></thead>
              <tbody>{queue.map((item) => <tr key={item.planId} className={selected?.planId === item.planId ? 'review-row-selected' : ''} onClick={() => void selectReview(item.planId)} tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter') void selectReview(item.planId) }}>
                <td data-label="Chiến dịch"><span className="review-campaign-cell"><strong>{item.title ?? item.planId}</strong><small>{item.planId}</small></span></td>
                <td data-label="Nhóm chuyển tiếp">{item.escalationCategory ?? 'Service không cung cấp'}</td>
                <td data-label="Lý do" className="reason-cell">{item.reason ?? 'Service không cung cấp'}</td>
                <td data-label="Ưu tiên">{item.priority ?? 'Service không cung cấp'}</td>
                <td data-label="Người duyệt">{item.authority ?? 'Service không cung cấp'}</td>
                <td data-label="Thời gian">{item.createdAt ? formatDateTime(item.createdAt) : 'Service không cung cấp'}</td>
                <td data-label="Trạng thái"><StatusBadge value={item.status ?? 'PENDING'} /></td>
              </tr>)}</tbody>
            </table>
          </div>}
        </section>
        {selected && <><button className="drawer-scrim" type="button" aria-label="Đóng chi tiết hồ sơ" onClick={() => setSelected(null)} /><ReviewDetailPanel overrideReason={overrideReason} setOverrideReason={setOverrideReason} detail={selected} reason={reason} reasonError={reasonError} setReason={setReason} setReasonError={setReasonError} pendingAction={pendingAction} setPendingAction={setPendingAction} isActionPending={isActionPending} onConfirm={() => void confirmAction()} onClose={() => setSelected(null)} /></>}
      </div>
    </section>
  )
}
...
function ReviewDetailPanel({ overrideReason, setOverrideReason, detail, reason, reasonError, setReason, setReasonError, pendingAction, setPendingAction, isActionPending, onConfirm, onClose }: ReviewDetailPanelProps) {
  ...
  return <aside className="review-detail-panel" aria-labelledby="review-detail-title">
    <div className="review-detail-heading"><div><span className="section-kicker">Chi tiết thẩm định</span><h3 id="review-detail-title">{detail.title ?? detail.planId}</h3><p>{detail.planId} · {detail.status ?? 'Service cung cấp'}</p></div><button type="button" className="button button-secondary" onClick={onClose}>Đóng</button></div>
